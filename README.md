...
> 基于 CFR (Counterfactual Regret Minimization) 的纳什均衡求解器，含自适应层。
...
- \(r \in \mathbb{N}\) 为当前回合数（0-indexed）。
  ...
  招式空间 \(\mathcal{M}\) 包含有限个动作。当启用 `反弹` 机制时，\(\mathcal{M}\) 增加一个 `反弹` 动作，游戏在两种模式下分别独立学习。每个招式 \(a \in \mathcal{M}\) 关联属性三元组 \(\operatorname{Attr}(a) = \langle \operatorname{cost}(a), \operatorname{atk}(a), \operatorname{def}(a) \rangle\)。判定函数 \(J: \mathcal{M} \times \mathcal{M} \to \{-1,0,1\}\) 定义为：
...
J(a_i, a_j) = 
\begin{cases}
1, & \text{if } \operatorname{atk}(a_i) > \operatorname{def}(a_j) \text{ and no special exemption}, \\
-1, & \text{if } \operatorname{atk}(a_j) > \operatorname{def}(a_i) \text{ and no special exemption}, \\
...
$$
q' = \max\!\bigl(0,\; \min(q - \operatorname{cost}(a) + \mathbb{I}_\text{bre},\; Q_{\max})\bigr)
$$
...
$$
\hat{P}_\text{pl}(m \mid S) = \frac{\operatorname{count}(S, m) + \alpha}{\sum_{m' \in \mathcal{V}_\text{pl}(S)} \operatorname{count}(S, m') + \alpha \cdot |\mathcal{V}_\text{pl}(S)|}
$$
...
$$
\lambda = \lambda_{\max} \cdot \frac{n}{n + k}
$$

其中 \(\lambda_{\max}\) 为最大混合率，\(k\) 为信任阈值。观察次数越多，越信任对手模型。
...
$$
P_{post}(m) = \frac{\displaystyle\frac{\beta}{|\mathcal{V}_{pl}|} + \operatorname{count}(m)}{\beta + \text{total}}
$$

其中 total 为当前对局中该状态已观察到的总动作数（实际上局内学习不区分状态，使用全局统计）。基于后验分布计算最佳响应和 `Softmax` 分布（与跨局类似）得到 \(\sigma_{br}^{within}\)。然后与当前策略 \(\sigma_{mid}\)（跨局修正后的）进行第二次混合：

$$
\sigma_{final}(a_i) = (1 - \lambda_{within}) \cdot \sigma_{mid}(a_i) + \lambda_{within} \cdot \sigma_{br}^{within}(a_i)
$$

其中 \(\lambda_{within} = \lambda_{within,\max} \cdot \displaystyle\frac{v}{v + \beta}\)，即随观测增加而增加。这里 \[\lambda_{within,\max}\] 是最大混合比例，为的是控制生成质量。
